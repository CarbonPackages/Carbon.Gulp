...
const PATHS = {
    all: FUNCTIONS.shureArray(config.root.watch).map(value =>
        path.join(config.root.base, "**", value, `_all${SUBSET_MATCHER}`)
    ),
    allsub: FUNCTIONS.shureArray(config.root.watch).map(value =>
        path.join(config.root.base, "**", value, `_allsub${SUBSET_MATCHER}`)
    ),
    fusion: FUNCTIONS.shureArray(config.root.watch).map(value =>
        path.join(config.root.base, "**", value, `_allFusion${SUBSET_MATCHER}`)
    )
};
