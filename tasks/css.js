...
if (Array.isArray(assetConf.loadPaths)) {
    assetConf.loadPaths = assetConf.loadPaths.map(value =>
        path.join(config.root.base, config.root.dest, value)
    );
} else {
    assetConf.loadPaths = path.join(
        config.root.base,
        config.root.dest,
        assetConf.loadPaths
    );
}
assetConf.relative = path.join(
    config.root.base,
    config.root.dest,
...
saasConfig.imagePath =
    (config.tasks.css.dest ? "../" : "") + saasConfig.imagePath;

function css() {
    return gulp
        .src(paths.src, { since: cache.lastMtime("css") })
        ...
        .pipe(
            mode.beautify
                ? beautify(config.tasks.css.cssbeautifyOptions)
                : util.noop()
        )
        .pipe(
